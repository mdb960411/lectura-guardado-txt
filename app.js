...
async function sendRecordToDrive(record, showSuccessToast = false) {
  ...
  try {
    ...
    // Apps Script puede crear correctamente el archivo, pero el navegador
    // no siempre recibe la confirmación desde el iframe de Google.
    // En ese caso no mostramos un falso error ni reenviamos automáticamente.
    if (message.includes('no respondió dentro del tiempo esperado')) {
      record.driveStatus = 'unconfirmed';
      record.driveError = 'Solicitud enviada a Google Drive; el navegador no recibió la confirmación.';
      await dbPut(record);
      showToast(`Solicitud TXT enviada para el código ${record.code}.`);
    } else {
